...
if [[ "$1" == "format" ]]; then
  ...
  bash cmake_dev.sh -lus -b $CONFIGURATION -r build_jobs_coverage_prepare -c $USE_CC -- $CRYPTO_OPTIONS \
  ...
  lcov --directory $PWD --capture --output-file coverage-full.info --ignore-errors gcov --rc geninfo_unexecuted_blocks=1
