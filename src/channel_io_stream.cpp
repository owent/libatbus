...
static void io_stream_on_recv_read_fn(uv_stream_t *stream, ssize_t nread, const uv_buf_t * /*buf*/) {
  ...
  // 网络错误
  if (nread < 0) {
    io_stream_channel_callback(io_stream_callback_event_t::EN_FN_RECEIVED, channel, conn_raw_ptr,
  ...
  if (nullptr == data || 0 == swrite) {
    ...
    while (buff_left_len > sizeof(uint32_t)) {
      ...
      if (buff_left_len >= sizeof(uint32_t) + vint_len + msg_len) {
        ...
        io_stream_channel_callback(io_stream_callback_event_t::EN_FN_RECEIVED, channel, conn_raw_ptr, 0, errcode,
  ...
  if (is_free) {
    if (conn_raw_ptr->read_head.len > 0) {
      io_stream_channel_callback(io_stream_callback_event_t::EN_FN_RECEIVED, channel, conn_raw_ptr, 0,
...
static void io_stream_handle_on_close(uv_handle_t *handle) {
  ...
  io_stream_channel_callback(io_stream_callback_event_t::EN_FN_DISCONNECTED, channel, iter->second.get(), 0,
...
static void io_stream_tcp_connection_cb(uv_stream_t *req, int status) {
  ...
  io_stream_channel_callback(io_stream_callback_event_t::EN_FN_ACCEPTED, channel, conn_raw_ptr, conn.get(),
...
static void io_stream_dns_connection_cb(uv_getaddrinfo_t *req, int status, struct addrinfo *res) {
  ...
  if (0 != listen_res && nullptr != async_data) {
    io_stream_channel_callback(io_stream_callback_event_t::EN_FN_CONNECTED, async_data->channel, async_data->callback,
...
int io_stream_listen(io_stream_channel *channel, const channel_address_t &addr, io_stream_callback_t callback,
  ...
  if (0 == UTIL_STRFUNC_STRNCASE_CMP("ipv4", addr.scheme.c_str(), 4) ||
    ...
    do {
      ...
      io_stream_channel_callback(io_stream_callback_event_t::EN_FN_CONNECTED, channel, callback, conn.get(), 0, ret,
                                 priv_data, priv_size);
      return ret;
    } while (false);
...
static void io_stream_on_written_fn(uv_write_t *req, int status) {
  ...
  while (true) {
    ...
    while (left_length > 0) {
      ...
      io_stream_channel_callback(io_stream_callback_event_t::EN_FN_WRITEN, connection->channel, connection, status,
  ...
  if (ATBUS_CHANNEL_IOS_CHECK_FLAG(connection->flags, io_stream_connection::EN_CF_CLOSING)) {
    while (!connection->write_buffer_manager.empty()) {
      ...
      while (left_length > 0) {
        ...
        io_stream_channel_callback(io_stream_callback_event_t::EN_FN_WRITEN, connection->channel, connection,
