...
#define CHECK_BUFFER(pointer, s, v)                                                                              \
  for (size_t i = 0; i < s; ++i) {                                                                               \
    ...
    if (*(reinterpret_cast<const unsigned char*>(pointer) + i) != static_cast<unsigned char>(v)) {               \
      std::cout << "[ RUNNING  ] buffer[" << i << "]=" << *(reinterpret_cast<const unsigned char*>(pointer) + s) \
                << ", expect=" << v << std::endl;                                                                \
      break;                                                                                                     \
    }                                                                                                            \
  }
...
CASE_TEST(buffer, varint) {
  ...
  CASE_EXPECT_EQ(1, res);
  CASE_EXPECT_EQ(0, j);
  CASE_EXPECT_EQ(0, atbus::detail::fn::read_vint(j, nullptr, sizeof(buf)));
  ...
  CASE_EXPECT_EQ(0, atbus::detail::fn::write_vint(i, nullptr, sizeof(buf)));
}

CASE_TEST(buffer, buffer_block) {
  // size align
  CASE_EXPECT_EQ(atbus::detail::buffer_block::full_size(99),
  ...
  CASE_EXPECT_EQ(atbus::detail::fn::buffer_next((void*)p->raw_data(), 50),
                 atbus::detail::fn::buffer_next((const void*)p->raw_data(), 50));
  CASE_EXPECT_EQ(atbus::detail::fn::buffer_prev((void*)p->data(), 50),
                 atbus::detail::fn::buffer_prev((const void*)p->data(), 50));
  ...
  // failed
  CASE_EXPECT_EQ(nullptr, atbus::detail::buffer_block::create(nullptr, 256, 256));
  CASE_EXPECT_EQ(nullptr, atbus::detail::buffer_block::destroy(nullptr));
  void* next_free = atbus::detail::buffer_block::create(buf, 256, 256);
  CASE_EXPECT_EQ(nullptr, next_free);
...
CASE_TEST(buffer, dynamic_buffer_manager_bf) {
  ...
  CASE_EXPECT_NE(nullptr, pointer);
  ...
  CASE_EXPECT_EQ(nullptr, pointer);
...
CASE_TEST(buffer, static_buffer_manager_bf) {
  ...
  CASE_EXPECT_EQ(atbus::detail::buffer_block::padding_size(1025) - 1,
                 mgr.limit().cost_size_ + atbus::detail::buffer_block::head_size(next_sz) +
                     3 * atbus::detail::buffer_block::head_size(256));
  CASE_EXPECT_EQ(4, mgr.limit().cost_number_);
...
CASE_TEST(buffer, static_buffer_manager_circle_bf) {
  ...
  res = mgr.push_back(check_ptr[3], 384 - atbus::detail::buffer_block::head_size(100) -
                                        atbus::detail::buffer_block::head_size(412) -
                                        atbus::detail::buffer_block::padding_size(1));
  CASE_EXPECT_EQ(EN_ATBUS_ERR_SUCCESS, res);
  CASE_EXPECT_EQ(4, mgr.limit().cost_number_);
...
CASE_TEST(buffer, static_buffer_manager_fb) {
  ...
  CASE_EXPECT_EQ(nullptr, pointer);
  res = mgr.push_front(
      pointer, 255 - atbus::detail::buffer_block::head_size(255) - atbus::detail::buffer_block::padding_size(1));
  CASE_EXPECT_EQ(EN_ATBUS_ERR_SUCCESS, res);
  CASE_EXPECT_NE(nullptr, pointer);

  CASE_EXPECT_EQ(1023 - atbus::detail::buffer_block::padding_size(1),
                 mgr.limit().cost_size_ + atbus::detail::buffer_block::head_size(255) +
                     3 * atbus::detail::buffer_block::head_size(256));
  CASE_EXPECT_EQ(4, mgr.limit().cost_number_);
...
CASE_TEST(buffer, static_buffer_manager_circle_fb) {
  ...
  CASE_EXPECT_EQ(nullptr, pointer);
  res = mgr.push_front(
      pointer, s = 255 - atbus::detail::buffer_block::head_size(255) - atbus::detail::buffer_block::padding_size(1));
  CASE_EXPECT_EQ(EN_ATBUS_ERR_SUCCESS, res);
  CASE_EXPECT_NE(nullptr, pointer);
  memset(pointer, -1, s);
  ...
  res = mgr.push_front(check_ptr[3], 384 - atbus::detail::buffer_block::head_size(100) -
                                         atbus::detail::buffer_block::head_size(412) -
                                         atbus::detail::buffer_block::padding_size(1));
  CASE_EXPECT_EQ(EN_ATBUS_ERR_SUCCESS, res);
  CASE_EXPECT_EQ(4, mgr.limit().cost_number_);
...
CASE_TEST(buffer, dynamic_buffer_manager_merge) {
  // merge back
  {
    ...
    CASE_EXPECT_EQ(nullptr, check_ptr[0]);
    CASE_EXPECT_EQ(1, mgr.limit().cost_number_);

    res = mgr.merge_back(check_ptr[1], 0);
    CASE_EXPECT_EQ(0, res);
    CASE_EXPECT_EQ(nullptr, check_ptr[1]);
    CASE_EXPECT_EQ(1, mgr.limit().cost_number_);
...
CASE_TEST(buffer, static_buffer_manager_merge_back) {
  // merge back : head NN tail ... => head NN NN tail ...
  {
    ...
    CASE_EXPECT_EQ(s + sr, mgr.front()->raw_size());
    CASE_EXPECT_EQ(pointer, mgr.front()->raw_data());
    CASE_EXPECT_EQ(check_ptr[2], atbus::detail::fn::buffer_next(mgr.front()->raw_data(),
                                                                atbus::detail::buffer_block::padding_size(s)));
    CASE_EXPECT_EQ(1, mgr.limit().cost_number_);
  ...
  // merge back : ... head NN tail ... => NN tail ... head NN ...
  {
    ...
    CASE_EXPECT_NE(pointer, mgr.front()->raw_data());
    CASE_EXPECT_EQ(check_ptr[1], atbus::detail::fn::buffer_next(mgr.front()->raw_data(),
                                                                atbus::detail::buffer_block::padding_size(sr)));
    CASE_EXPECT_EQ(1, mgr.limit().cost_number_);
...
CASE_TEST(buffer, static_buffer_manager_merge_front) {
  // merge front : NN tail ... head NN ... => NN tail ... head NN NN ...
  {
    ...
    CASE_EXPECT_EQ(pointer, atbus::detail::fn::buffer_next(mgr.front()->raw_data(),
                                                           atbus::detail::buffer_block::padding_size(120)));
    CHECK_BUFFER(mgr.front()->raw_data(), s, 0xec);
  }
  ...
  // merge front : .... head NNNNNN tail .... => .... head NN NNNNNN tail ....
  {
    ...
    CASE_EXPECT_EQ(pointer, atbus::detail::fn::buffer_next(mgr.front()->raw_data(),
                                                           atbus::detail::buffer_block::padding_size(136)));
    CHECK_BUFFER(mgr.front()->raw_data(), sr, 0xeb);
  }
