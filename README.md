...
CMake 目标依赖的代码和资源（包括生成、复制及非手写文件）禁止无条件 `touch`，也不得在内容未变化时覆盖写入。
生成规则应准确声明 `OUTPUT`、`BYPRODUCTS`、`DEPENDS` 和 `DEPFILE`，并使用内容稳定的生成方式或临时输出加
`cmake -E copy_if_different`。只有不参与编译或打包的独立 stamp 文件可以无条件更新时间戳。
